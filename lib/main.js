...
var omxdirector = function () {
  ...
  /**
   * sendAdvanced description: send any of the additional command line flags
   * @param  {String} flag  e.g., -o|--pos (type omxplayer at the command prompt for a full list of flags)
   * @param  {String} value The value for the flag
   * @return {void}       Does not return anything
   */
  var sendFlag = function(flag, value){
    if (omxProcess) {
      try {
        omxProcess.stdin.write(flag + ' ' + value, function (err) {
          console.log(err);
        });
      } catch (err) {
        console.log(err);
